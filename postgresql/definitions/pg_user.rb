define :pg_user, :action => :create do
  ...
  when :create
    ...
    privileges[:superuser] = params[:superuser] if params[:superuser]
    privileges[:createdb] = params[:createdb] if params[:createdb]
    privileges[:login] = params[:login] if params[:login]
